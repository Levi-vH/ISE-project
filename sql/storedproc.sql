...
BEGIN
	...
	SET @sql =	N'
				...
				FROM		WORKSHOP W INNER JOIN
							...
							WHERE WORKSHOP_ID IN (SELECT DISTINCT(W.WORKSHOP_ID) FROM WORKSHOP W
							...
							ELSE
								BEGIN 
									SET @sql += ' WHERE (D.VOORNAAM IS NULL OR D.VOORNAAM LIKE ''%%'') AND (D.ACHTERNAAM IS NULL OR D.ACHTERNAAM LIKE ''%%''))'
								END
...
BEGIN
	...
	IF	EXISTS	(SELECT SECTORNAAM FROM ADVISEUR WHERE SECTORNAAM = @sectornaam) OR
		EXISTS	(SELECT SECTORNAAM FROM AANVRAAG WHERE SECTORNAAM = @sectornaam) OR
		EXISTS	(SELECT SECTORNAAM FROM DEELNEMER WHERE SECTORNAAM = @sectornaam) OR
		EXISTS	(SELECT SECTORNAAM FROM WORKSHOP WHERE SECTORNAAM = @sectornaam)
		BEGIN
...
BEGIN
	...
	DECLARE @sql NVARCHAR(4000)
	SET @sql =	N'
				SELECT		WT.TYPE, COUNT(W.TYPE) AS AANTAL
				FROM		WORKSHOP W
				RIGHT JOIN WORKSHOPTYPE WT ON W.TYPE = WT.TYPE 
				GROUP BY	WT.TYPE
				'
	EXEC sp_executesql @sql
END
...
BEGIN
	...
	DECLARE @year SMALLINT = (SELECT JAAR FROM BESCHIKBAARHEID WHERE WORKSHOPLEIDER_ID = @workshopleader)
	DECLARE @quarter CHAR(1) = (SELECT KWARTAAL  FROM BESCHIKBAARHEID WHERE WORKSHOPLEIDER_ID = @workshopleader)

	UPDATE	BESCHIKBAARHEID
	SET		AANTAL_UUR = (AANTAL_UUR - (CAST(DATEDIFF(minute, @workshopstarttime , @workshopendtime) AS NUMERIC(5,2)) / 60.00))
	WHERE	WORKSHOPLEIDER_ID = @workshopleader
	AND		JAAR = @year
...
@contactperson_name				NVARCHAR(80) = NULL,
@contactperson_email			NVARCHAR(100) = NULL,
@contactperson_phonenumber		NVARCHAR(12) = NULL
)
AS
BEGIN
	SET NOCOUNT ON
...
BEGIN
	...
	IF @column = 'BEVESTIGING_WORKSHOPLEIDER'
		BEGIN
			DECLARE @workshopleader_ID INT = (SELECT WORKSHOPLEIDER FROM MODULE_VAN_GROEP WHERE GROEP_ID = @groep_id and MODULENUMMER = @modulenummer)
			DECLARE @start_workshop TIME(7) = (SELECT STARTTIJD FROM MODULE_VAN_GROEP WHERE GROEP_ID = @groep_id and MODULENUMMER = @modulenummer)
			DECLARE @end_workshop TIME(7) = (SELECT EINDTIJD FROM MODULE_VAN_GROEP WHERE GROEP_ID = @groep_id and MODULENUMMER = @modulenummer)
			DECLARE @year SMALLINT = (SELECT CAST(YEAR(DATUM) AS SMALLINT) FROM MODULE_VAN_GROEP WHERE GROEP_ID = @groep_id and MODULENUMMER = @modulenummer)
			DECLARE @quarter CHAR(1) = (SELECT CAST(DATEPART(QUARTER, DATUM) AS CHAR(1)) FROM MODULE_VAN_GROEP WHERE GROEP_ID = @groep_id and MODULENUMMER = @modulenummer)
